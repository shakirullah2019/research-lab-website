...
CREATE INDEX IF NOT EXISTS idx_research_featured ON research(featured) WHERE featured = TRUE;
...
-- RLS Policies (DROP then CREATE to allow re-run)
DROP POLICY IF EXISTS "Public read research" ON research;
...
DROP POLICY IF EXISTS "Admin all research" ON research;
CREATE POLICY "Admin all research" ON research FOR ALL USING (
  auth.uid() IN (SELECT id FROM profiles WHERE role = 'admin')
);
DROP POLICY IF EXISTS "Admin all publications" ON publications;
...
DROP POLICY IF EXISTS "Admin all media" ON media_files;
...
-- Seed Data (ON CONFLICT DO NOTHING to allow re-run)
